...
void InitReplacementState() {
    ...
    for (size_t i = 0; i < block_meta.size(); i++)
        block_meta[i].dead_ctr = 1; // neutral
...
uint32_t GetVictimInSet(
    ...
    // Evict block with dead_ctr==0 if any
    for (uint32_t way = 0; way < LLC_WAYS; way++) {
        size_t idx = get_block_meta_idx(set, way);
        if (block_meta[idx].dead_ctr == 0) {
            dead_evictions++;
            return way;
        }
    }
    // Else evict LRU (way 0)
    return 0;
}
...
void UpdateReplacementState(
    ...
    if ((access_counter & 0x3FF) == 0) {
        for (size_t i = 0; i < block_meta.size(); i++)
            if (block_meta[i].dead_ctr > 0)
                block_meta[i].dead_ctr--;
    }
    ...
    // On hit: increment dead_ctr, promote SHiP counter
    if (hit) {
        if (meta.dead_ctr < 3) meta.dead_ctr++;
        if (ship_entry.counter < 3) ship_entry.counter++;
        hits++;
        return;
    }

    // On miss: insertion depth policy
    ...
    if (ship_entry.counter >= 2) {
        // High reuse signature: insert MRU (simulate BIP, way 15)
        meta.dead_ctr = 3;
        ship_mru_inserts++;
    } else {
        // Low reuse signature: insert LRU (simulate LIP, way 0)
        meta.dead_ctr = 1;
        ship_lru_inserts++;
    }

    // DIP insertion override for blocks with neutral signature
    if (ship_entry.counter == 1) {
        if (use_bip) {
            // BIP: insert at MRU (way 15) with 1/32 probability, else LRU (way 0)
...
void PrintStats() {
    std::cout << "DIP-SHiP Hybrid with Dead-Block Approximation\n";
...
void PrintStats_Heartbeat() {
    std::cout << "DIP-SHiP+Dead heartbeat: accesses=" << access_counter
              << ", hits=" << hits
