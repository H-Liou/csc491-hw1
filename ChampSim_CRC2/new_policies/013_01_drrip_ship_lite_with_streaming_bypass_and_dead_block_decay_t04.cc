...
void UpdateReplacementState(
    ...
    if (streaming) {
        // Do not insert into cache (simulate as if block is not filled)
        // But update dead counter for victim
    ...
    uint8_t ins_rrpv = 2; // SRRIP: insert at long re-reference
    if (!use_srrip) {
        // BRRIP: insert at distant (3) with low probability, else at 2
