...
#define DECAY_PERIOD (LLC_SETS * LLC_WAYS / 2)
...
uint32_t GetVictimInSet(
    ...
    // Dead block bypass: prefer block with dead_ctr==3
    for (uint32_t way = 0; way < LLC_WAYS; ++way)
...
void UpdateReplacementState(
    ...
    if (hit) {
        ...
        dead_ctr[set][way] = 0; // block is live
        return;
    } else {
    ...
    // --- DIP insertion depth selection (LIP/BIP) ---
    uint8_t insertion_rrpv = 3; // LIP default: insert at long RRPV
    bool use_bip = false;
    if (is_lip_leader[set]) {
        ...
        use_bip = (psel < (1 << 9)); // favor LIP if psel < 512
    }
    if (use_bip) {
        insertion_rrpv = (rand() % 100 < 5) ? 2 : 3; // BIP: 5% at 2, 95% at 3
    }
    ...
    // --- Adaptive streaming and dead block bypass ---
    // If streaming detected OR dead block likely, AND weak SHiP reuse, bypass
    if ((streaming || dead_ctr[set][way] == 3) && ship_ctr[set][way] <= 1) {
    ...
    dead_ctr[set][way] = 0; // reset dead counter
...
void PrintStats() {
    int strong_reuse = 0, total_blocks = 0, streaming_sets = 0, dead_blocks = 0;
    for (uint32_t s = 0; s < LLC_SETS; ++s) {
    ...
    std::cout << "DSDS Policy: DIP-SHiP Hybrid + Dead Block + Streaming Decay" << std::endl;
