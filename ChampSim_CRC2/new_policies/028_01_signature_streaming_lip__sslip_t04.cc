...
void UpdateReplacementState(
    ...
    if (!use_lip) {
        // BIP: insert at LRU (3), but 1/32 inserts at MRU (0)
    ...
    // ---- Streaming detector: streaming sets force LIP (insert at LRU)
    if (streaming)
        insertion_rrpv = 3;
...
void PrintStats() {
    ...
    std::cout << "SSLIP Policy: Signature-Streaming LIP" << std::endl;
    ...
    std::cout << "Final PSEL: " << psel << " (0=BIP, 1023=LIP)" << std::endl;
}
...
void PrintStats_Heartbeat() {
    ...
    std::cout << "PSEL (heartbeat): " << psel << " (0=BIP, 1023=LIP)" << std::endl;
}