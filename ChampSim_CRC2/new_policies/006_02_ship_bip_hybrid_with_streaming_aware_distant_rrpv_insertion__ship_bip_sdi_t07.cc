...
void UpdateReplacementState(
    ...
    if (!hit) {
        ...
        // SHiP-bias: If signature shows high reuse, always MRU insert
        if (ship_ctr == 3) {
            line_meta[set][way].rrpv = 0; // MRU
        }
        // Streaming detected: insert at distant (RRPV_MAX)
        else if (streaming) {
            line_meta[set][way].rrpv = RRPV_MAX; // NOT bypass: fill, but easy victim
        }
        // Otherwise use set-dueling policy (SRRIP or BIP)
        else {
            // BIP: mostly insert at RRPV_MAX, sometimes MRU (1/32)
            if (!use_srrip) {
                static uint32_t bip_ctr = 0;
                if ((++bip_ctr & 0x1F) == 0)
                    line_meta[set][way].rrpv = 0; // rare MRU
                else
                    line_meta[set][way].rrpv = RRPV_MAX;
            } else {
...
void PrintStats() {
    std::cout << "SHIP-BIP-SDI Policy: SHiP-BIP Hybrid with Streaming-Aware Distant RRPV Insertion" << std::endl;
    uint64_t total_fills = 0, streaming_fills = 0, mrufills = 0;
    for (uint32_t set = 0; set < LLC_SETS; ++set) {
        for (uint32_t way = 0; way < LLC_WAYS; ++way) {
            total_fills++;
    ...
    std::cout << "Fraction streaming-region distant-inserts: "
              << (double)streaming_fills / total_fills << std::endl;
    std::cout << "Fraction MRU-inserts (SHiP-predicted): "
