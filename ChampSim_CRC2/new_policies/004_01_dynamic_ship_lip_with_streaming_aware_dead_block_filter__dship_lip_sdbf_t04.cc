...
void UpdateReplacementState(
    ...
    // For leader sets, use DIP set-dueling (LIP/BIP)
    if (is_leader_set[set] == 1) {
        insert_rrpv = 0; // LIP: always insert at MRU
    } else if (is_leader_set[set] == 2) {
        insert_rrpv = (rand() % 32 == 0) ? 0 : RRPV_MAX; // BIP: insert at MRU 1/32, else distant
    } else if (psel < (1 << (PSEL_BITS - 1))) {
        insert_rrpv = 0; // follower: LIP
    } else {
        insert_rrpv = (rand() % 32 == 0) ? 0 : RRPV_MAX; // follower: BIP
    }
...
void PrintStats() {
    std::cout << "DSHIP-LIP-SDBF Policy: SHiP-lite signature insertion + DIP set-dueling (LIP/BIP) + streaming-aware dead-block filter\n";
}
