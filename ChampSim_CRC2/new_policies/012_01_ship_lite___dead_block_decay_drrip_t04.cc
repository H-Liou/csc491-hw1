...
#define DEAD_COUNTER_BITS 1 // 1 bit per block
#define DEAD_DECAY_PERIOD 5000000 // Decay every N accesses
...
// Dead-block decay: clear all dead flags periodically
...
uint32_t GetVictimInSet(
    ...
    // Priority: dead blocks first, else RRIP
    for(uint32_t w=0; w<LLC_WAYS; w++)
        if(blocks[set][w].dead)
            return w;
...
void UpdateReplacementState(
    ...
    // On hit: set block to MRU, clear dead flag, increment SHiP counter
    if(hit) {
    ...
    // On fill/replace: SHiP outcome update for victim block, set victim dead
    uint16_t victim_sig = blocks[set][way].signature;
    if(ship_table[victim_sig] > 0)
        ship_table[victim_sig]--;
    blocks[set][way].dead = 1; // Mark replaced block as dead
    ...
    blocks[set][way].signature = sig;
    blocks[set][way].dead = 0; // New fill is not dead
}
...
void PrintStats() {
    ...
    std::cout << "SHiP-Lite+DeadBlock: Dead blocks=" << dead_blocks << std::endl;
}
