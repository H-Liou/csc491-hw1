...
// --- Find victim: RRPV, prefer dead blocks ---
...
void UpdateReplacementState(
    ...
    uint8_t ins_rrpv = 3; // LIP: always insert at distant
    if (!use_lip) {
    ...
    // --- Streaming: insert streaming blocks always at distant RRPV ---
    if (streaming)
        ins_rrpv = 3;
...
void PrintStats() {
    std::cout << "SHiP-Lite + LIP/BIP Set-Dueling + Streaming Distant Insertion + Dead-Block Counter: Final statistics." << std::endl;
