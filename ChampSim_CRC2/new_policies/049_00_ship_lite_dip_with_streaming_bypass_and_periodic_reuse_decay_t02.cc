...
//---------------------------------------------
// DIP set-dueling: 32 leader sets, 6-bit PSEL
#define NUM_LEADER_SETS 32
#define PSEL_MAX 63
uint8_t PSEL = PSEL_MAX/2; // 6-bit
...
//---------------------------------------------
// Mini-reuse counter: 2 bits per block, periodic decay
uint8_t reuse_ctr[LLC_SETS][LLC_WAYS]; // 0–3

//---------------------------------------------
// SHiP-Lite: 1024-entry signature table, 2 bits per entry, 4-bit signature
#define SHIP_SIG_TABLE_SIZE 1024
uint8_t ship_sig_table[SHIP_SIG_TABLE_SIZE]; // 2 bits per entry
...
void UpdateReplacementState(
    ...
    //--- SHiP-Lite signature extraction (4-bit signature)
    uint32_t sig = ((PC >> 2) ^ set) & 0xF;
    uint32_t sig_idx = ((set << 4) | sig) & (SHIP_SIG_TABLE_SIZE-1);

    //--- Periodic decay of reuse counters (every 4096 fills)
    static uint64_t global_fill_count = 0;
    global_fill_count++;
    if ((global_fill_count & 0xFFF) == 0) { // every 4096 fills
    ...
    if (hit) {
        ...
        if (set_type[set] == 0) { // LIP: always insert at 3
            ins_rrpv = 3;
        } else if (set_type[set] == 1) { // BIP: insert at 3 except 1/32 at 2
        ...
        // SHiP-Lite bias: if signature is hot, insert at 1/0
    ...
    if (!hit && victim_addr) {
        uint32_t victim_sig = ((victim_addr >> 2) ^ set) & 0xF;
        uint32_t victim_sig_idx = ((set << 4) | victim_sig) & (SHIP_SIG_TABLE_SIZE-1);
...
void PrintStats() {
    ...
    std::cout << "SHiP-Lite DIP + Streaming Bypass + Periodic Reuse Decay Policy" << std::endl;
    ...
    std::cout << "PSEL: " << (int)PSEL << "/" << PSEL_MAX << std::endl;
}
...
void PrintStats_Heartbeat() {
    ...
    std::cout << "PSEL (heartbeat): " << (int)PSEL << "/" << PSEL_MAX << std::endl;
}