...
void UpdateReplacementState(
    ...
    uint8_t ins_rrpv = 0; // default MRU
    if (use_lip) {
        ins_rrpv = 3; // LIP: always insert at LRU
    } else {
        // BIP: insert at LRU only 1/32 fills, else MRU
        ins_rrpv = ((rand() % 32) == 0) ? 3 : 0;
    }

    // --- Streaming: insert at LRU if streaming detected ---
    if (streaming)
        ins_rrpv = 3;
...
void PrintStats() {
    std::cout << "SHiP-Lite + DIP Set-Dueling + Streaming-Insertion Bias: Final statistics." << std::endl;
