...
struct SLDC_BlockMeta {
    uint8_t rrpv;       // 2 bits
    uint8_t pc_sig;     // 6 bits
    uint8_t dead_ctr;   // 2 bits
};
SLDC_BlockMeta block_meta[LLC_SETS][LLC_WAYS];
...
// DIP constants
const uint8_t BIP_INSERT_PROB = 31; // 1/32 chance for BIP MRU insertion
...
void UpdateReplacementState(
    ...
    // --- Streaming detector ---
    SLDC_StreamSet &ss = stream_sets[set];
    ...
    // --- Dead-block counter ---
    uint8_t &dead_ctr = block_meta[set][way].dead_ctr;
    ...
    // --- On miss (new insertion) ---
        ...
        // Dead-block counter decay (approximate deadness)
        if (dead_ctr > 0)
            dead_ctr--;
        ...
        // Streaming: always use LIP (insert at LRU)
        if (ss.streaming && ss.window > 0) {
            insert_rrpv = RRIP_LRU;
        } else {
            // SHiP-guided + dead-block counter insertion
            if (ship_table[sig_idx] >= 2 && dead_ctr >= 2)
                insert_rrpv = RRIP_MRU; // hot signature, high reuse
            else if (dead_ctr == 0)
                insert_rrpv = RRIP_LRU; // dead block, insert at LRU
            else
                insert_rrpv = RRIP_MAX - 1; // intermediate

            // DIP: override insertion depth for non-leader sets
            if (!is_lip_leader && !is_bip_leader) {
                bool use_bip = (psel < 512);
                if (use_bip) {
                    // BIP: insert at MRU with low probability, else LRU
                    if ((rand() % BIP_INSERT_PROB) == 0)
                        insert_rrpv = RRIP_MRU;
                    else
                        insert_rrpv = RRIP_LRU;
                }
                // else use LIP: always insert at LRU
                else {
                    insert_rrpv = RRIP_LRU;
                }
            }
        ...
        if (is_lip_leader && !hit && insert_rrpv == RRIP_MRU && psel < 1023)
            psel++;
        if (is_bip_leader && !hit && insert_rrpv == RRIP_LRU && psel > 0)
            psel--;
    }
}
...
void PrintStats() {
    ...
    for (uint32_t s = 0; s < LLC_SETS; ++s)
        if (stream_sets[s].streaming)
            streaming_sets++;
    std::cout << "SLDC-DIP: Streaming sets at end: " << streaming_sets << std::endl;
    std::cout << "SLDC-DIP: Final PSEL value: " << psel << std::endl;
}
