...
void InitReplacementState() {
    for (uint32_t s = 0; s < LLC_SETS; ++s) {
        for (uint32_t w = 0; w < LLC_WAYS; ++w) {
            ...
            repl_meta[idx].dead_ctr = 2; // neutral
...
void UpdateReplacementState(
    ...
    if (hit) {
        ...
        repl_meta[idx].outcome = 1;
        if (repl_meta[idx].dead_ctr < 3)
            repl_meta[idx].dead_ctr++; // More reuse
    } else {
        ...
        if (SHIP_table[signature] >= 2 || repl_meta[idx].dead_ctr >= 2) {
            insert_rrpv = 0; // likely reusable
        } else if (SHIP_table[signature] == 1 || repl_meta[idx].dead_ctr == 1) {
        ...
        repl_meta[idx].dead_ctr = 1; // new block: cold
    }
