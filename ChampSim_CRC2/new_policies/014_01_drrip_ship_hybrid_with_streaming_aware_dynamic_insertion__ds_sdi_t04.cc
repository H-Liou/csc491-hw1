...
void UpdateReplacementState(
    ...
    // DRRIP: followers use PSEL to choose SRRIP or BRRIP
    if (is_leader_set[set] == 0) {
        if (PSEL >= (1 << (PSEL_BITS - 1))) {
            insertion_rrpv = 2; // SRRIP
        } else {
            insertion_rrpv = 3; // BRRIP (insert at LRU)
        }
    }
    ...
    // Streaming detector: if streaming detected and SHiP ctr<=1, insert at distant RRPV (LRU)
...
void PrintStats() {
    ...
    std::cout << "DS-SDI Policy: DRRIP + SHiP-lite + Streaming-Aware Dynamic Insertion" << std::endl;
