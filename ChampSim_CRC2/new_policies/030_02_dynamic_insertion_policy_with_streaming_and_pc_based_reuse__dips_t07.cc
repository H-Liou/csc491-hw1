...
// ---- DIP-style set-dueling: Leader sets and PSEL ----
#define NUM_LEADER_SETS 32
uint8_t is_LIP_leader[LLC_SETS]; // 0: follower, 1: LIP leader, 2: BIP leader
uint16_t psel = 512; // 10-bit PSEL, mid-point
...
void InitReplacementState() {
    ...
    // Assign leader sets (round-robin)
    for (uint32_t i = 0; i < NUM_LEADER_SETS; ++i) {
        // Even: LIP leader, Odd: BIP leader
        is_LIP_leader[i] = 1;           // First N/2 sets: LIP
        is_LIP_leader[NUM_LEADER_SETS + i] = 2; // Next N/2 sets: BIP
    }
    access_counter = 0;
    psel = 512;
}
...
void UpdateReplacementState(
    ...
    // ---- DIP set-dueling outcome update ----
    // Only count misses in leader sets
    if (!hit) {
        if (is_LIP_leader[set] == 1 && psel < 1023)
            psel++;
        else if (is_LIP_leader[set] == 2 && psel > 0)
            psel--;
    }

    // ---- Insertion policy ----
    uint8_t insertion_rrpv = 3; // Default: bypass (LRU)
    if (streaming) {
        insertion_rrpv = 3; // streaming sets always bypass
    } else {
        // Leader sets: fixed policy
        if (is_LIP_leader[set] == 1) {
            insertion_rrpv = 3; // LIP: always LRU
        } else if (is_LIP_leader[set] == 2) {
            insertion_rrpv = 2; // BIP: 1/32 at MRU, else LRU
            if ((access_counter % 32) == 0)
                insertion_rrpv = 0;
        } else {
            // Follower sets: choose LIP or BIP by psel
            bool do_LIP = (psel >= 512);
            if (do_LIP)
                insertion_rrpv = 3;
            else {
                insertion_rrpv = 2;
        ...
        // SHiP bias: if signature strongly reused, override to MRU
...
void PrintStats() {
    ...
    std::cout << "DIPS Policy: Dynamic Insertion Policy with Streaming & SHiP" << std::endl;
