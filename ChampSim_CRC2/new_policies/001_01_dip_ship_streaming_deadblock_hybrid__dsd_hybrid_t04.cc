...
// DIP: 32 leader sets per policy, 6-bit PSEL
#define NUM_LEADER_SETS 32
#define PSEL_MAX 63
uint8_t PSEL = PSEL_MAX / 2;
...
// Per-block: 2-bit dead-block counter (reuse)
...
uint32_t GetVictimInSet(
    ...
    // Dead-block aware victim selection: prefer blocks with dead_counter==0 and max RRPV
    uint32_t victim = LLC_WAYS;
    for (uint32_t way = 0; way < LLC_WAYS; ++way) {
        if (rrpv[set][way] == 3 && dead_counter[set][way] == 0) {
            return way;
        }
        if (rrpv[set][way] == 3 && victim == LLC_WAYS)
            victim = way;
    }
    if (victim != LLC_WAYS)
        return victim;

    // If none, increment all RRPV and retry
    for (uint32_t way = 0; way < LLC_WAYS; ++way)
        rrpv[set][way]++;
    // On next call, will find victim
    return 0;
}
...
void UpdateReplacementState(
    ...
    // On fill: decide insertion policy (DIP)
    ...
    if (stream_conf[set] >= 2) {
        ...
        // Dead-block: if dead_counter==0, distant insert
        if (dead_counter[set][way] == 0)
            ins_rrpv = 3;
        // SHiP: if signature counter high, MRU insert
    ...
    // DIP leader sets update PSEL
    if (is_lip_leader(set)) {
        // Always LIP: distant insert
        ins_rrpv = 3;
        if (hit && PSEL < PSEL_MAX) PSEL++;
    } else if (is_bip_leader(set)) {
        // BIP: MRU insert with low probability (1/32)
        if ((rand() & 31) == 0) {
            ins_rrpv = 0;
        } else {
            ins_rrpv = 3;
        ...
        if (PSEL >= PSEL_MAX / 2) {
            ...
            // BIP: MRU insert with low probability
            if ((rand() & 31) == 0) {
                ins_rrpv = 0;
            } else {
                ins_rrpv = 3;
    ...
    // Decay SHiP counter on fill
    if (ship_counter[sig] > 0) ship_counter[sig]--;

    // Periodic decay of dead-block counters (every 4096 fills)
    static uint64_t fill_count = 0;
    fill_count++;
    if ((fill_count & 4095) == 0) {
...
// --- Stats ---
void PrintStats() {
    std::cout << "DSD Hybrid Policy: DIP (LIP/BIP) + SHiP-lite + Streaming + Dead-block, PSEL=" << int(PSEL) << std::endl;
}
