...
uint32_t GetVictimInSet(
    ...
    // Dead-block predictor: evict block with counter==3 if possible
    for (uint32_t way = 0; way < LLC_WAYS; ++way)
...
void UpdateReplacementState(
    ...
    if (hit) {
        // On hit, reset dead-block counter
        dead_block[set][way] = 0;
    } else {
        // On miss/evict, increment dead-block counter
        if (dead_block[set][way] < 3)
            dead_block[set][way]++;
    }
    ...
    if (use_brrip) {
        // BRRIP: insert at 2 with 1/32 probability, else at 3
    ...
    // ---- Dead-block predictor: blocks with counter==3 always insert at LRU (3)
    if (dead_block[set][way] == 3)
        insertion_rrpv = 3;

    // ---- Streaming detector: streaming sets bypass fill if score==3, else insert at LRU (3)
    if (streaming) {
    ...
    // ---- Periodic decay of SHiP and dead-block counters ----
...
void PrintStats() {
    ...
    for (int i = 0; i < LLC_SETS; ++i)
        for (int j = 0; j < LLC_WAYS; ++j)
            if (dead_block[i][j] == 3) dead_blocks++;
    std::cout << "ADSS Policy: Adaptive Dead-Block SHiP Streaming" << std::endl;
