...
StreamInfo stream_info[LLC_SETS];
...
void InitReplacementState() {
    for (uint32_t s = 0; s < LLC_SETS; ++s) {
        ...
        stream_info[s].last_delta = 0;
        stream_info[s].stream_count = 0;
    }
...
void UpdateReplacementState(
    ...
    StreamInfo &si = stream_info[set];
    int64_t cur_delta = (si.last_addr != 0) ? (int64_t)paddr - (int64_t)si.last_addr : 0;
    if (cur_delta == si.last_delta && cur_delta != 0) {
        // Streaming detected
        if (si.stream_count < 3) si.stream_count++;
    } else {
        if (si.stream_count > 0) si.stream_count--;
    }
    si.last_delta = cur_delta;
    si.last_addr = paddr;
    ...
    if (hit) {
        ...
        if (si.stream_count >= 2) {
            insert_rrpv = MAX_RRPV; // streaming: insert at distant RRPV, likely to bypass
        } else {
            // SHiP bias
