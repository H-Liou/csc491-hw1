...
#define DECAY_PERIOD 4096 // Decay dead counters every 4096 fills
...
// --- Find victim: RRIP with dead-block bias ---
uint32_t GetVictimInSet(
    ...
    // Prefer blocks with high dead_ctr (likely dead)
...
void UpdateReplacementState(
    ...
    // --- On hit: update SHiP-lite predictor, set RRPV=0, reset dead counter ---
    if (hit) {
    ...
    // --- SHiP-lite: bias toward MRU if signature is reused ---
    uint8_t ins_rrpv = 3; // default distant
    if (ship_table[sig] >= 2)
        ins_rrpv = 0; // MRU if predicted reused
    else if (srrip)
        ins_rrpv = 0; // SRRIP: always MRU
    else {
    ...
    // --- On eviction: update SHiP-lite predictor for victim block, increment dead counter ---
    ...
    global_fill_ctr++;
    if ((global_fill_ctr & (DECAY_PERIOD-1)) == 0) {
...
void PrintStats() {
    std::cout << "DRRIP-SHiP+DeadBlock: Final statistics." << std::endl;
    ...
    std::cout << "Dead blocks (counter=3): " << dead_blocks << std::endl;
