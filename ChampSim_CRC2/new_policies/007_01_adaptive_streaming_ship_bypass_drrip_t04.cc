...
bool IsStreamingAccess(uint32_t set, uint64_t paddr) {
    // Check both entries in stream_table[set]
    for (int i = 0; i < 2; ++i) {
        ...
        if (stream_table[set][i].last_delta != 0 &&
            ...
            // If stream_count high, treat as streaming
            return (stream_table[set][i].stream_count >= STREAM_DETECT_THRESHOLD);
        }
    }
    // Not continuing any stream: replace LRU entry
...
void PrintStats() {
    std::cout << "Adaptive Streaming-SHiP Bypass DRRIP: Final statistics." << std::endl;
