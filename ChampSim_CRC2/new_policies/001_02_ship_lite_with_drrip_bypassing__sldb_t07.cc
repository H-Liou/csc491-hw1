...
// Per-block metadata: RRIP, PC signature
struct SLDB_BlockMeta {
    uint8_t rrpv;       // 2 bits
    uint8_t pc_sig;     // 6 bits
};
SLDB_BlockMeta block_meta[LLC_SETS][LLC_WAYS];
...
// DRRIP leader sets: first 32 for SRRIP, next 32 for BRRIP
...
// DRRIP constants
const uint8_t BRRIP_INSERT_PROB = 31; // 1/32 chance for BRRIP long insertion
...
void UpdateReplacementState(
    ...
    // --- Streaming detector ---
    SLDB_StreamSet &ss = stream_sets[set];
    ...
    // --- On miss (new insertion) ---
        ...
        if (ss.streaming && ss.window > 0) {
            ...
            // DRRIP set-dueling: leader sets
            bool is_srrip_leader = (set < SRRIP_LEADER_SETS);
            bool is_brrip_leader = (set >= SRRIP_LEADER_SETS && set < NUM_LEADER_SETS);
            ...
            // DRRIP: override insertion depth for non-leader sets
            if (!is_srrip_leader && !is_brrip_leader) {
                // Use PSEL to choose SRRIP or BRRIP
                bool use_srrip = (psel >= 512);
                if (!use_srrip) {
                    // BRRIP: insert at RRIP_DISTANT with low probability, else RRIP_MAX
            ...
            if (is_srrip_leader && !hit && insert_rrpv == RRIP_MRU && psel < 1023)
                psel++;
            if (is_brrip_leader && !hit && insert_rrpv == RRIP_MAX && psel > 0)
                psel--;
        }
    }
}
...
void PrintStats() {
    ...
    for (uint32_t s = 0; s < LLC_SETS; ++s)
        if (stream_sets[s].streaming)
            streaming_sets++;
    std::cout << "SLDB: Streaming sets at end: " << streaming_sets << std::endl;
    std::cout << "SLDB: Final PSEL value: " << psel << std::endl;
}
