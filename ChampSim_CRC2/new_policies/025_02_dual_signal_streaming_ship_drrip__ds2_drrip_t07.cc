...
void UpdateReplacementState(
    ...
    uint8_t insertion_rrpv = 2; // default SRRIP (long insertion)

    // Set-dueling for leader sets
    ...
    if (leader == 1) // SRRIP leader
        ...
        use_brrip = (psel < 512) ? false : true;

    if (use_brrip) {
        // BRRIP: insert at distant RRPV with low probability (1/32)
    ...
    // ---- Streaming detector: streaming sets insert at distant RRPV or bypass outright
    if (streaming) {
        // If streaming score is high, bypass SHiP/DRRIP and insert at RRPV=3 (LRU)
        insertion_rrpv = 3;
        // Optionally: bypass fill if streaming score is saturated (score==3)
        // For simplicity, always insert at 3 if streaming.
    }
    ...
    if (!hit) {
        if (leader == 1 && !streaming) { // SRRIP leader, not streaming
            if (psel < 1023) psel++;
        }
        else if (leader == 2 && !streaming) { // BRRIP leader, not streaming
            if (psel > 0) psel--;
        }
...
void PrintStats() {
    ...
    std::cout << "DS2-DRRIP Policy: Dual-Signal Streaming-SHiP DRRIP" << std::endl;
