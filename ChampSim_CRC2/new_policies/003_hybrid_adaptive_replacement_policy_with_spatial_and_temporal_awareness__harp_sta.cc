...
void InitReplacementState() {
    for (uint32_t set = 0; set < LLC_SETS; ++set) {
        for (uint32_t way = 0; way < LLC_WAYS; ++way) {
            replacement_state[set][way] = {0, 0, 0, 0};
        }
    }
}
...
uint32_t GetVictimInSet(
    ...
    // Calculate scores based on recency, frequency, spatial locality, and phase alignment
    for (uint32_t way = 0; way < LLC_WAYS; ++way) {
        ...
        // Weighted score: prioritize recency, spatial locality, and penalize phase mismatch
        uint64_t score = recency_score + (frequency_score * 2) + (spatial_score * 3) + (phase_mismatch * 1000);
...
void UpdateReplacementState(
    ...
    if (hit) {
        // Update recency, frequency, and spatial score on hit
