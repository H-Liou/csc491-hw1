...
uint32_t GetVictimInSet(
    ...
    // Prefer blocks predicted dead (dead_block == 1)
    for (uint32_t way = 0; way < LLC_WAYS; ++way)
...
void UpdateReplacementState(
    ...
    // ---- Dead-block approximation: predicted dead blocks insert at LRU (3) or bypass fill
    if (dead_block[set][way] == 1) {
        // If block predicted dead, insert at LRU (3)
        insertion_rrpv = 3;
    }
...
void PrintStats() {
    ...
    for (int i = 0; i < LLC_SETS; ++i)
        for (int j = 0; j < LLC_WAYS; ++j)
            if (dead_block[i][j] == 1) dead_blocks++;
    std::cout << "SDD Policy: SHiP-DeadBlock DRRIP" << std::endl;
