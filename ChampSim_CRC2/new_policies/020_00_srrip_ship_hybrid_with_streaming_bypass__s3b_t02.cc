...
void UpdateReplacementState(
    ...
    // --- Adaptive insertion/bypass ---
    // If streaming detected AND weak SHiP reuse, bypass (do not insert)
    if (streaming && ship_ctr[set][way] <= 1) {
        // Leave block invalid (simulate bypass by setting RRPV=3 and not updating signature/counter)
        rrpv[set][way] = 3;
        ship_signature[set][way] = sig;
        ship_ctr[set][way] = 1;
        return;
    ...
    uint8_t insertion_rrpv = 3; // SRRIP default: insert at distant RRPV
...
void PrintStats() {
    ...
    std::cout << "S3B Policy: SRRIP + SHiP-lite + Streaming Bypass" << std::endl;
