...
#define STREAM_DETECT_THRESHOLD 3 // If 3 consecutive monotonic deltas, treat as streaming
...
// --- Streaming detector: update per-set history ---
inline bool update_stream_detector(uint32_t set, uint64_t paddr) {
    stream_state_t &st = stream_table[set];
    uint64_t delta = (st.last_addr == 0) ? 0 : paddr - st.last_addr;
    bool monotonic = (st.last_addr != 0) && (delta == st.last_delta) && (delta != 0);
    if (monotonic) {
        if (st.delta_cnt < STREAM_DELTA_MAX) st.delta_cnt++;
    } else {
        st.delta_cnt = 0;
    }
    st.last_delta = delta;
    st.last_addr = paddr;
    return (st.delta_cnt >= STREAM_DETECT_THRESHOLD);
}
...
void UpdateReplacementState(
    ...
    bool is_streaming = update_stream_detector(set, paddr);
    ...
    if (is_streaming) {
        // Bypass: mark block as invalid, do not cache
...
void PrintStats() {
    ...
    for (uint32_t s = 0; s < LLC_SETS; s++) {
        if (stream_table[s].delta_cnt >= STREAM_DETECT_THRESHOLD)
            streaming_sets++;
    }
    std::cout << "SL-SB: Streaming sets=" << streaming_sets << "/" << LLC_SETS << std::endl;
    std::cout << "SL-SB: PSEL=" << PSEL << "/" << PSEL_MAX << std::endl;
    std::cout << "SL-SB: Leader sets: SHiP=" << ship_leader_cnt << " BRRIP=" << brrip_leader_cnt << std::endl;
}
