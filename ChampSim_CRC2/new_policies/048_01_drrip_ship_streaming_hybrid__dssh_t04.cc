...
#define PSEL_BITS 10
uint16_t psel = 512; // 10-bit, neutral start
...
void InitReplacementState() {
    ...
    for (uint32_t set = 0; set < LLC_SETS; ++set) {
        ...
        last_addr[set] = 0;
        stream_score[set] = 0;
        is_leader_set[set] = false;
    }
    // SHiP-lite: initialize signature counters
    for (uint32_t i = 0; i < SIG_TABLE_SIZE; ++i)
        sig_ctr[i] = 1; // neutral initial value
    // DRRIP: select leader sets (first half for SRRIP, second half for BRRIP)
    for (uint32_t i = 0; i < NUM_LEADER_SETS; ++i)
        is_leader_set[(i * LLC_SETS) / NUM_LEADER_SETS] = true;
    psel = 512;
    access_counter = 0;
}
...
void UpdateReplacementState(
    ...
    // --- DRRIP leader set policy selection ---
    bool leader = is_leader_set[set];
    bool use_srrip = false;
    if (leader) {
        // First half of leader sets use SRRIP, second half use BRRIP
        use_srrip = (set < LLC_SETS / 2);
    } else {
        // Non-leader sets: use SRRIP if PSEL >= 512, else BRRIP
        use_srrip = (psel >= 512);
    }

    // --- Insertion depth logic ---
    if (is_streaming) {
        // Streaming detected: bypass with probability 1/4, else insert at distant
        if ((PC ^ paddr) & 0x3) {
        ...
        // Strong signature: insert at MRU
        rrpv[set][way] = 0;
    }
    else {
        // Weak/unseen signature: use DRRIP policy
        if (use_srrip) {
            rrpv[set][way] = 2; // SRRIP: distant
        } else {
            // BRRIP: insert at distant (2) with low probability, else LRU (3)
    ...
    if (leader && !is_streaming) {
        // Only update PSEL for non-streaming accesses
        if (hit) {
            if (set < LLC_SETS / 2 && psel < ((1<<PSEL_BITS)-1))
                psel++; // SRRIP leader hit
            else if (set >= LLC_SETS / 2 && psel > 0)
                psel--; // BRRIP leader hit
        }
    }
...
void PrintStats() {
    ...
    std::cout << "DSSH: sig_ctr==2: " << sig2 << " / " << SIG_TABLE_SIZE << std::endl;
    std::cout << "DSSH: sig_ctr==3: " << sig3 << std::endl;
    int stream_sets = 0;
    for (uint32_t set = 0; set < LLC_SETS; ++set)
        if (stream_score[set] >= STREAM_DETECT_THRESH)
            stream_sets++;
    std::cout << "DSSH: Streaming sets detected: " << stream_sets << " / " << LLC_SETS << std::endl;
    std::cout << "DSSH: PSEL: " << psel << std::endl;
}
