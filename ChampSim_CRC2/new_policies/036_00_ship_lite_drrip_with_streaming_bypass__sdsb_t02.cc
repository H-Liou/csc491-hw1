...
uint32_t GetVictimInSet(
    ...
    // Streaming detected: bypass (simulate by returning invalid way)
    if (is_streaming[set]) {
        // Indicate bypass by returning LLC_WAYS (no insertion)
        return LLC_WAYS;
    }
...
void UpdateReplacementState(
    ...
    // If streaming detected, bypass insertion
    if (is_streaming[set] && !hit) {
        return; // Do not update cache state for bypassed block
    }
...
void PrintStats() {
    std::cout << "SHiP-Lite DRRIP with Streaming Bypass: Final statistics." << std::endl;
