...
// --- Streaming detector: per-set stride pattern ---
uint64_t last_addr[LLC_SETS];
uint8_t stream_score[LLC_SETS]; // 2 bits per set

// --- DIP-style set-dueling: 64 leader sets, PSEL ---
...
void UpdateReplacementState(
    ...
    uint16_t sig = (uint16_t)((PC >> 2) & 0x3F);
    ...
    uint8_t insertion_rrpv = 3; // LIP: always insert at LRU
    if (!use_lip) {
        // BIP: insert at LRU, but 1/32 at MRU (0)
...
void PrintStats() {
    ...
    std::cout << "DSS-LIP Policy: Dynamic SHiP-LIP Streaming Replacement" << std::endl;
