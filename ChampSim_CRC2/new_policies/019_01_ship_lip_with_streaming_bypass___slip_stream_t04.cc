...
// ---- DIP Metadata ----
// 8-bit PSEL counter for set-dueling (LIP vs BIP)
static uint8_t psel = 128; // 8 bits, initialized mid-range

// 32 leader sets: 16 for LIP, 16 for BIP
...
inline bool IsStreaming(uint32_t set, uint64_t paddr) {
    ...
    last_addr[set] = paddr;
    // Streaming if score >= 24 (faster response than previous 32)
    return stream_score[set] >= 24;
}
...
void UpdateReplacementState(
    ...
    if (signature_outcome[sig] >= 2) {
        ...
        uint8_t ins_rrpv = 3; // LIP: always insert at LRU (RRPV=3)
        if (is_lip_leader) {
            ins_rrpv = 3; // LIP
        } else if (is_bip_leader) {
            // BIP: insert at MRU only 1/32 times, else at LRU
            ...
            ins_rrpv = (psel >= 128) ? 3 : ((rand() % 32 == 0) ? 0 : 3);
        }
...
void PrintStats() {
    ...
    for (uint32_t i = 0; i < LLC_SETS; ++i)
        if (stream_score[i] >= 24) ++streaming_sets;
    std::cout << "SLIP-Stream Policy\n";
    std::cout << "Strong reuse signatures: " << strong_sig << " / 64\n";
    std::cout << "Streaming sets: " << streaming_sets << " / " << LLC_SETS << "\n";
    std::cout << "PSEL: " << (uint32_t)psel << " (LIP if >=128, BIP if <128)\n";
}
