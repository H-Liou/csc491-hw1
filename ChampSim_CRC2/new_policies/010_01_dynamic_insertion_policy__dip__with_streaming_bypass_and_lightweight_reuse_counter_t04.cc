...
void UpdateReplacementState(
    ...
    // --- On hit: set RRPV to 0, increment reuse counter ---
    if (hit) {
        rrpv[set][way] = 0;
    ...
    // Streaming detector: if streaming, bypass (insert at distant)
    if (streaming) {
        ins_rrpv = 3;
    }

    // Reuse counter: if block had poor reuse, force distant insertion
...
void PrintStats() {
    std::cout << "DIP with Streaming Bypass and Lightweight Reuse Counter: Final statistics." << std::endl;
    std::cout << "PSEL: " << PSEL << std::endl;
}

// Print periodic (heartbeat) statistics
void PrintStats_Heartbeat() {
    // Optionally print PSEL, reuse counter histogram, streaming stats
}