...
// --- Dead-Block Decay Period ---
#define DBC_DECAY_PERIOD 5000000
uint64_t dbc_decay_tick = 0;
...
void InitReplacementState() {
    ...
    memset(dead_ctr, 0, sizeof(dead_ctr)); // DBC = 0 (not dead)
...
void UpdateReplacementState(
    ...
    // Dead-Block Decay Tick (periodic)
    dbc_decay_tick++;
    if ((dbc_decay_tick % DBC_DECAY_PERIOD) == 0) {
        decay_dead_counters();
    }

    uint8_t sig = get_signature(PC);

    // On hit: promote block, increment reuse counter, clear dead counter
    if (hit) {
        rrpv[set][way] = 0;
    ...
    if (ship_ctr[set][way] >= 2 && dead_ctr[set][way] == 0)
        insertion_rrpv = 0;

    // --- Dead-block bias: if DBC high, always insert at distant RRPV ---
    if (dead_ctr[set][way] == 3)
        insertion_rrpv = STREAM_BYPASS_RRPV;

    // --- Streaming bypass: if streaming detected, insert at distant RRPV and increment all DBCs in set ---
    if (streaming) {
        insertion_rrpv = STREAM_BYPASS_RRPV;
        // Aggressively mark set as dead (streaming)
        for (uint32_t w = 0; w < LLC_WAYS; ++w)
            if (dead_ctr[set][w] < 3)
                dead_ctr[set][w]++;
    }
    ...
    // Dead-block update: if not hit, increment DBC (but not above 3)
    if (dead_ctr[set][way] < 3)
        dead_ctr[set][way]++;
...
void PrintStats() {
    ...
    std::cout << "SRRIP-SHiP-DBCSD Policy: SRRIP/BRRIP set-dueling + SHiP-lite + Dead-Block Counter + Streaming Decay" << std::endl;
    std::cout << "Blocks with strong reuse (SHIP ctr==3): " << strong_reuse << "/" << total_blocks << std::endl;
    std::cout << "Blocks marked dead (DBC==3): " << dead_blocks << "/" << total_blocks << std::endl;
...
void PrintStats_Heartbeat() {
    ...
    std::cout << "Strong reuse blocks (heartbeat): " << strong_reuse << "/" << total_blocks << ", Dead blocks: " << dead_blocks << std::endl;
}