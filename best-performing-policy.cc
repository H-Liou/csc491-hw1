...
// Per-set: which leader set, or 2 (follower)
...
//---------------------------------------------
// Dead-block bit: 1 bit per block
uint8_t dead_bit[LLC_SETS][LLC_WAYS]; // 0: live, 1: dead
...
//---------------------------------------------
// Find victim in the set (dead-block first, then RRIP)
...
void UpdateReplacementState(
    ...
    if (hit) {
        ...
        dead_bit[set][way] = 0; // was reused: live
    } else {
        ...
        // Leader sets: set insertion policy
        ...
        dead_bit[set][way] = 0; // filled: assume live
    }
...
void PrintStats() {
    ...
    std::cout << "DRRIP + Streaming Bypass + Dead-Block Bit Policy" << std::endl;
